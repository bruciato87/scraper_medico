...
export async function sendEmail(type, data = null) {
  ...
  if (!apiKey || apiKey === 're_test_key') {
    console.log(`[INFO] Resend API key not set. Skipping email send for type: ${type}`);
    return false;
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      console.log(`[SUCCESS] Email inviata tramite Resend con successo: ${resData.id}`);
      return true;
    } else {
      const errText = await response.text();
      console.error(`[ERROR] Invio email fallito da Resend: Status ${response.status} - ${errText}`);
      return false;
    }
  } catch (error) {
