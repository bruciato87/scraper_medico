...
export async function scrapeGPAvailability() {
  ...
  try {
    ...
    // Check if we are already on the GP search page
    let alreadyOnGPPage = false;
    try {
      const currentUrl = page.url();
      if (currentUrl.includes('scelta-revoca') || currentUrl.includes('sceltarevoca') || currentUrl.includes('scelta-e-revoca')) {
        for (const sel of inputSelectors) {
          if (await page.locator(sel).first().isVisible()) {
            alreadyOnGPPage = true;
            break;
          }
        }
      }
    } catch (e) {
      alreadyOnGPPage = false;
    }
