...
export async function scrapeGPAvailability() {
  ...
  try {
    ...
    if (alreadyOnGPPage) {
      console.log('[INFO] Il browser è già sulla pagina di Scelta e Revoca. Salto la navigazione iniziale.');
    } else {
      ...
      try {
        const gpLink = page.locator('a:has-text("Scelta e revoca del medico"), a[href*="scelta-e-revoca"], [role="link"]:has-text("Scelta e revoca del medico")').first();
        if (await gpLink.isVisible()) {
          console.log('[INFO] Trovato link di Scelta e Revoca. Clic in corso...');
          await gpLink.click();
          clickNavigated = true;
          await delay(6000);
        }
      } catch (e) {
        console.log('[INFO] Impossibile cliccare il link per Scelta e Revoca, eseguo navigazione diretta...');
      }
      ...
      console.log('[INFO] Attesa rendering del form di ricerca (main page o iframe)...');
      let found = false;
      for (let i = 0; i < 20; i++) {
        // Check main frame
        for (const sel of inputSelectors) {
      ...
      if (!targetFrame) {
        ...
        if (currentUrl.includes('login') || (currentUrl.includes('fserl-pubblica/') && !currentUrl.includes('scelta-e-revoca-del-medico'))) {
          ...
          console.log('[WARNING] Form di ricerca non trovato in nessun frame.');
          throw new Error('session_expired');
        }
      }
    }
    ...
    for (const city of municipalities) {
      ...
      for (const sel of inputSelectors) {
        try {
          const loc = targetFrame.locator(sel).first();
      ...
      if (!cityInput) {
        ...
        cityInput = targetFrame.locator('input').first();
      }
      ...
      try {
        const dropdownItem = targetFrame.locator(`li:has-text("${city}"), [role="option"]:has-text("${city}"), div:has-text("${city}")`).first();
