...
export async function scrapeGPAvailability() {
  ...
  // RESTORE SESSION STORAGE via init script
  if (state.sessionStorage) {
    await context.addInitScript((storages) => {
      window.addEventListener('DOMContentLoaded', () => {
        try {
          const currentUrl = window.location.href;
          const match = storages.find(s => currentUrl.startsWith(s.url) || s.url.includes(window.location.hostname));
