...
export async function scrapeGPAvailability() {
  ...
  try {
    ...
    for (const city of municipalities) {
      ...
      // If we are currently displaying results, click the "Indietro" button to return to the search form
      const backBtn = targetFrame.locator('a:has-text("Indietro"), .indietro').first();
      if (await backBtn.isVisible()) {
        console.log('[INFO] Rilevato stato risultati. Ritorno alla form di ricerca...');
        await backBtn.click();
        await delay(2000);
      }
